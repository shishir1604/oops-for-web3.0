...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function mulDiv(uint p, uint r,uint z) public pure returns(uint){
        return p*r/z;
    }
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
        ...
        uint256  R=rate;
        uint  T=time;
        //uint256 n=uint256(10^18);
        while (T>0)
        {
            P=P+mulDiv(P,R,100);// to calculate simple interest;
            T--;
        }
        return P;
    }
    ...
    function viewDues(address adr) isOwner()public view returns(uint256){
        return loans[adr];
    }
    function settleDues(address adr) isOwner()public returns(bool){
        bool t=sendCoin(adr,loans[adr],tx.origin);
        if (t==false)
        return t;
        else{
            loans[adr]=0;
    ...
    // implement viewDues and settleDues which allow *ONLY* the owner to *view* and *settle* his loans respectively. They take in the address of a creditor as arguments. viewDues returns a uint256 corresponding to the due amount, and does not modify any state variables.
